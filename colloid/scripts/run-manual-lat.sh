...
# echo 14900 | sudo tee /sys/devices/system/node/node3/hugepages/hugepages-2048kB/nr_hugepages
...
# for x in 0 0.5 1; do
    ...
    PYTHONPATH=$PYTHONPATH:$mio_path python3 -m mio $config-iso-x$x-mio --stats_colloidlat &
    pid_stats=$!;
    sleep $duration;
    # killall record_stats;
    # killall python3;
...
# for x in 0 1; do
    ...
    PYTHONPATH=$PYTHONPATH:$mio_path python3 -m mio $config-bg-x$x-mio --ant_cpus $stream_core_list --ant_num_cores $stream_num_cores --ant_mem_numa 3 --ant stream --ant_writefrac 50 --ant_inst_size 64 --ant_duration $(($duration+20)) --stats_colloidlat &
    pid_mio=$!;
    sleep 10;
